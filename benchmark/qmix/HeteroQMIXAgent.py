...
class HeteroQMIXAgent:
    ...
    """
    def __init__(self, env, cfg: HeteroQMIXcfg, device: Optional[str] = None):
    ...
    # -------------------------
    # QMIX loss (team reward)
    # -------------------------                                                                                    
    def _loss_team_qmix(self, batch, *, is_ue: bool,update_vnorm: bool=True) -> torch.Tensor:
        ...
        if is_ue:
            ...
            mix, mix_tgt = self.ue_mix, self.ue_mix_tgt
            act_dim = self.ue_act_dim
            vnorm = self.ue_vnorm
        else:
            net, tgt = self.bs_net, self.bs_tgt
            mix, mix_tgt = self.bs_mix, self.bs_mix_tgt
        ...
        for i in range(N):
            ...
            for t in range(L):
                ...
                if t==0:
                    a_prev_1hot = torch.zeros(B, act_dim, device=self.device)
                else:
                    a_prev_1hot = one_hot(a_i[:,t-1], act_dim)  # (B, A)
                
                # online update at time t
                q_all, h = net(obs_t, a_prev_1hot, h)  # (B, A), (B, hidden_dim)
                ...
                with torch.no_grad():
                    ...
                    # Double Q: next action from online net
        ...
        for t in range(L):
            q_tot_t, _ = mix(agent_qs[:, t], state[:, t])
            tq_tot_t, _ = mix_tgt(target_qs[:, t], next_state[:, t])
    ...
    def update(self) -> Dict[str, float]:
        ...
        loss_ue = self._loss_team_qmix(batch_ue, is_ue=True, update_vnorm=True)
        self.opt_ue.zero_grad()
        loss_ue.backward()
        nn.utils.clip_grad_norm_(list(self.ue_net.parameters()) + list(self.ue_mix.parameters()), self.cfg.grad_clip)
        self.opt_ue.step()
        
        loss_bs = self._loss_team_qmix(batch_bs, is_ue=False, update_vnorm=True)
