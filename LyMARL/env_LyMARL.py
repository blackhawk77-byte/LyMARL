...
class MAPPOEnvironment:
    def __init__(self,
        ...
        self.bs_action_dim = self.bs_top_k + 1  # last index NONE
        ...
        # tracked for logging only (not used in observations)
        self.prev_req_ratio = {bs.bs_id: 0.0 for bs in self.base_stations}

        # previous slot power for interference modeling (cache)
        ...
        # local obs dim: [Q(1) + rates(n_bs) + Zb(n_bs)]
        self.local_obs_dim = 1 + 2 * self.n_bs

        # BS obs dim: [Z_b] + score*K
        ...
        print(f"  V={self.V} | power_budget_ratio(ρ)={self.power_budget_ratio} | bs_over_penalty(c)={self.bs_over_penalty}")
        print(f"  UE reward(team): mean_u[ served_rate_u * Q_u(t+1) ]  (eta_q not used in team reward)")
        print(f"  per-user reward: served_rate_u - eta_q*Q_u   (for logging)")
        print(f"  BS reward(per BS): alpha*served_rate - c*max(0,on-rho)^2 - beta*Z_b*ON")
        print(f"  local_obs_dim={self.local_obs_dim} | bs_obs_dim={self.bs_obs_dim} | global_obs_dim={self.global_obs_dim}")
        print(f"  prev_req_ratio removed from UE local/global obs and BS obs")
        print(f"  ON-ratio removed from UE local obs, global obs, and BS local obs")
        print(f"  Remaining budget removed from BS local obs")
        print(f"{'='*92}\n")
    ...
    def calculate_scheduled_rate(self, user_id: int, serving_bs_id: int, tx_power_map: Dict[int, float]) -> float:
        ...
        - 간섭은 '이번 슬롯 ON된 BS들의 tx_power_map' 기준
        """
        user = next(u for u in self.users if u.ue_id == user_id)
    ...
    def step_joint(self, ue_actions: Dict[int, int], bs_actions: Dict[int, int], cand_lists: List[List[int]]):
        ...
        bs_served_rate = {bs.bs_id: 0.0 for bs in self.base_stations}   # per BS
        ...
        # update prev_power for next-step interference
        ...
        ue_per_user_rewards = {u.ue_id: float(served_rates[u.ue_id] - self.eta_q * self.Q_u[u.ue_id]) for u in self.users}
